...
namespace Life.Api.Controllers
{
    ...
    public class GameController : ControllerBase
    {
        ...
        public GameController(GameDbContext context, IOptions<GameSettings> settings)
        {
        ...
        /// </summary>
        /// <param name="boardState">The initial state of the board.</param>
        ...
        public async Task<ActionResult<Guid>> UploadBoardState([FromBody] bool[,] board, CancellationToken cancellationToken)
        {
            ...
            if (rows == 0 || rows > _settings.MaxBoardSize ||
                columns == 0 || columns > _settings.MaxBoardSize)
            {
                return BadRequest($@"Invalid board state. The board must have at least one cell and at most
                    {_settings.MaxBoardSize}x{_settings.MaxBoardSize} cells.");
            }
